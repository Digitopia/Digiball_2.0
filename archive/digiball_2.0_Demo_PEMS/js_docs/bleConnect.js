...
function drawBLE() {
    ...
    //para reconectar o ble display

    let tSize = constrain(width / 70, 12, 20)
    textSize(tSize)
    textAlign(RIGHT);

    text("reconectar", width - width / 44, height - 30);
}

// para reconectar o ble -- através de mousepressed
function BLEMousecheck() {
    if (mouseY < height && mouseY > height - 30 * 1.5 && mouseX < width - width / 40 && mouseX > width - width / 40 * 5) {
        connectToBle()
    }
}
